...
exports.getEventPublic = asyncHandler(async (req, res) => {
  ...
  const event = await Event.findOne({ slug, status: { $ne: 'cancelled' } })
    .select('name clientName date time venue description dressCode googleMapsUrl invitationVideo coverImage status slug');
...
exports.getGuestInvitation = asyncHandler(async (req, res) => {
  ...
  const event = await Event.findOne({ slug })
    .select('name clientName date time venue description dressCode googleMapsUrl invitationVideo coverImage status');
